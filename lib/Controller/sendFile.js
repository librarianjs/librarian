...
function sendFile (req, res, next) {
  ...
  let cache = (p, name) => {
    return Promise.resolve(p).then(data => {
      this.cache.put(name, data).catch(e => console.error(e))
      return data
    })
  }
  ...
  filePromise = this.cache.get(filename).then(file => {
    ...
    return cache(this.storage.get(filename), filename)
  })
  ...
  if (filename !== id) {
    filePromise = filePromise.then(() => {
     ...
     return cache(this.storage.get(id), id)
    })
