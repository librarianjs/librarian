...
class Api {
  ...
  async _file_buffer (id, params) {
    ...
    if (!file && name !== id) {
      ...
      if (modified) {
        await Promise.all([
          // TODO josh: we should avoid saving the transformed images to
          // storage. This is what caches are for
          this.storage.put(name, modified),
          this.cache.put(name, modified),
        ])
        file = modified
      }
    }
